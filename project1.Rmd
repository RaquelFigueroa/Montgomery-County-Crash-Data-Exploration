...
With the great number of "N/A", "Other", and "Unknown" values, there will definitely need to be some data cleaning. In particular, these values were deleted from graphs which depended on visualizations based on a particular value like with categorical data. For example, these exact values were discarded when mapping the locations of collisions resulting in injuries, property damage, or fatalities. 
