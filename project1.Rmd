...
During the project, we will be investigating the times in the day that collision occur using military time, and we will also be exploring any correlations between weather and the serenity of the accident. Furthermore, we will see who causes the most accidents locks or out of town and which agencies had the most filled out reports. 


## Project Data

The dataset what we will be using is about each collision indecent that happened on county and local roadways within the Montgomery County. The data collected was from the Automated Crash Reporting System by many agencies such as Montgomery County Police, Gaithersburg Police and more. Within the dataset, it gives where the collision happened such as the latitude and longitude, the date and the street type. It always has information about outside factors such as weather, surface condition, lighting, and traffic control. Information about the other drivers was recorded such as driver license state, substance abuse, distracted drivers, drivers at fault, and injury severity. Other factors that were published were about vehicle damage, vehicle movement, the type of vehicle. Some of the information that will be used will include the location, latitude, longitude, the driver's license state, severity, and weather. 
...
We are creating a visual map of where the accidents happened and what was the ACRS Report type. We used the same zoomed up map from the last graph wanted to see where the three different ACRS Report types of fatal crashes, injury crashes, and property damage crashes.  It appears that property damage crashes had the most accidents, then injury crashes and the fatal crashes had the least. Most of the property damage crashes were mostly located not on the main roads, but it might have been neighborhoods. Furthermore, injury crashes mostly happened on the main roads of the county. We imagined that fatal crashes involved at least someone could have been hospitalized, so we wanted to know where happened and we marked them with an X. We found out that the majority of the fatal crashes also happened in the county. However, there was still a lot on the outskirts of the county. 
